...
#This shell script uses a NCL code to draw contours on atmospheric levels.
...
unset rmcounter
 ...
 echo -e "\n Drawing contour maps for $contourselect ..."
	...
	*) echo "Output Images Will Be 'X11'"; imgfmt="x11";;
    ...
    if [[ ${imgfmt} == "x11" ]]; then
     ...
     cnpostname=`echo $wrfout2 | cut -d "_" -f2-3`
      if [ -z "$outname2" ]; then
       outname=`echo "contour-level-"$contourselect"-"$cnpostname`
