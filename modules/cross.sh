...
#This shell script uses a some NCL codes to draw cross-sections of WRF file variables.
...
unset rmcounter
   ...
   echo -e "\nDrawing cross-section maps for $crossselect ..."
    echo -e " \nMethod of Cross-Section: 1, 2, or 3?\n"
     select crossmode in "One-point-and-an-angle" "Two-points"
      do
      ...
      echo ""
      read -p "Enter the height (kilometer) of the cross-section (press Enter for the default value (10 km)): " crossheight
        #   if [ -z ${crossheight} ]; then
           if [[  -z ${crossheight} || ${crossheight} -gt 25 || ${crossheight} -le 0 ]] 2> /dev/null; then
            echo " The height of the cross-section will be set to 10 km"
            crossheight=10
           fi
      export crossheight
...
select colpal in "Rainbow-start_from_blue" "Rainbow-start_from_red" "Rainbow+white-start_from_white" "Blue..Red" "White..Blue" "White..Red" "White..Green" "White..Yellow..Orange..Red" "White..Black" "Black..White"
    ...
    if [[ ${imgfmt} == "x11" ]]; then
     ...
     cnpostname=`echo $wrfout2 | cut -d "_" -f2-3`
      if [ -z "$outname2" ]; then
       outname=`echo "cross-section-"$crossselect"-"$cnpostname`
    ...
    export outname
     ...
     if [[ ${imgfmt} == "x11" ]]; then
      if [[ ${crossmode} == "One-point-and-an-angle" ]]; then
       ncl modules/cross1.ncl
      else
       ncl modules/cross2.ncl
      fi
     else
       ...
       if [[ ${crossmode} == "One-point-and-an-angle" ]]; then
        ncl ../modules/cross1.ncl
	if [[ ${imgfmt} == "animated_gif" ]]; then
         convert $outname.000001.png path-$outname.jpg
	 rm $outname.000001.png
